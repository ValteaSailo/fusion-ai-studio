...
import { getAuth, signInWithEmailAndPassword, createUserWithEmailAndPassword, onAuthStateChanged, signOut, updateProfile, signInWithCustomToken, GoogleAuthProvider, signInWithPopup } from 'firebase/auth';
...
export default function App() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // Save initial user data to Firestore if it doesn't exist
      if (db) {
      ...
      {showAuth && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center p-4 selection:bg-orange-600/40 font-sans bg-black/80 backdrop-blur-sm animate-in fade-in duration-300 ease-out">
          ...
          <div className="w-full max-w-md bg-[#131826] border border-white/5 rounded-[28px] p-8 shadow-[0_20px_60px_rgba(0,0,0,0.8)] relative z-10 animate-in zoom-in-95 duration-300 ease-out">
            ...
            <button type="button" onClick={handleGoogleLogin} className="focus:outline-none w-full bg-white/5 hover:bg-white/10 border border-white/10 text-white text-[13px] font-bold py-3 rounded-xl transition-all duration-200 flex items-center justify-center gap-2 mt-5 shadow-sm hover:shadow-md hover:-translate-y-[1px]">
